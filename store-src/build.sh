...
# Pages re-redirects ".html" URLs to their pretty form, so rewriting /store to
# /store.html via _redirects loops forever (ERR_TOO_MANY_REDIRECTS). Serving the
# page as store/index.html lets every static host answer /store directly from
# the asset tree, with no redirect machinery involved. Root store.html stays as
# a legacy alias (hosts with pretty-URL handling 301 it to /store themselves).
mkdir -p store
