...
- **_redirects:**
  ...
  - NO trailing-slash rules. The 6 hub trailing-slash 301s (`/composer/`, `/work/`, `/reel/`, `/process/`, `/services/`, `/contact/` -> canonical without slash) were retired on 2026-09-19: Cloudflare Pages answers the slashless `/work` with its own 308 to `/work/`, so the 301 pointed straight back into that 308 (`/work -> 308 /work/ -> 301 /work -> ...` ERR_TOO_MANY_REDIRECTS) and every hub link, including the whole primary nav on /store, was dead. Both forms are now 200s: `<route>/index.html` serves `/route/`, and the generated root twin `<route>.html` serves the canonical `/route` exactly the way `store.html` always served `/store` (tools/route-aliases.mjs).
...
**Template Control:**
- All routes are REAL static paths: /work/index.html, /reel/index.html, etc., not rewrites - no redirect loops (store had ERR_TOO_MANY_REDIRECTS before, fixed via static path; the hubs repeated it through trailing-slash 301s against Pages' directory 308, retired 2026-09-19 - see _redirects)
- Every route ships a byte-identical root twin (`/work.html` beside `/work/index.html`, the way `/store` has always shipped `store.html`) so the canonical slashless URL answers 200 with no redirect hop and no rule can loop. Generated by `tools/route-aliases.mjs`, parity-enforced by `tools/check-sitemap.mjs`.
...
- [x] _redirects fixed: removed /* /index.html 200 soft-404 trap, 301 legacy .html->pretty, and (2026-09-19) the 6 hub trailing-slash 301s that looped against Cloudflare Pages' directory 308
